...
print(f'  Média: {d.mean():.4f} km/km²')
print(f'  Mediana: {d.median():.4f} km/km²')
...
print(f'  Média: {d.mean():.4f} km/1000hab')
print(f'  Mediana: {d.median():.4f} km/1000hab')
print(f'  Min: {d.min():.4f} | Max: {d.max():.4f}')
idx_max = d.idxmax()
print(f'  Mun com maior: {mun.loc[idx_max, "NM_MUN"]} ({d.max():.4f})')
print()
...
for i, row in top5.iterrows():
    print(f'  {row["NM_MUN"]}: {row["Dens_Area_Total"]:.3f} km/km² ({row["Ext_Total"]:.1f} km em {row["Area_km2"]:.0f} km²)')

print()
print('TOP 5 Densidade População:')
top5 = mun.nlargest(5, 'Dens_Pop_Total')[['NM_MUN', 'Dens_Pop_Total', 'Populacao', 'Ext_Total']]
for i, row in top5.iterrows():
    print(f'  {row["NM_MUN"]}: {row["Dens_Pop_Total"]:.3f} km/1000hab ({row["Ext_Total"]:.1f} km para {row["Populacao"]:.0f} hab)')
...
print(f'Densidade Área Estadual: {ext_total/area_total:.4f} km/km²')
print(f'Densidade Pop Estadual: {ext_total/(pop_total/1000):.4f} km/1000hab')
...
for tipo, col in [('Concessionada', 'Ext_Concessionada'), ('Não Concessionada', 'Ext_Não_Concessionada')]:
    ext = mun[col].sum()
    dens_area = ext / area_total
    dens_pop = ext / (pop_total / 1000)
    print(f'{tipo}: {ext:,.2f} km | Dens.Área: {dens_area:.4f} km/km² | Dens.Pop: {dens_pop:.4f} km/1000hab')
